...
import { createHomeByCode, getmccBycode } from '@/api/homePage';
...
const Poolaadd = (props) => {
  ...
  const handnleAdd = async () => {
    ...
    await createHomeByCode(params).then((res) => {
      if (res.success) {
  ...
  const handleSearch = () => {
    ...
    console.log(form.getFieldsValue().designCode);
    ...
    getmccBycode(form.getFieldsValue().designCode).then((res) => {
      if (res.success) {
        setImages(res.images[3].imageUrl);
                ...
                <Row>
                  ...
                  <div>
                    ...
                    <div style={{ marginTop: '5px', marginLeft: '20px' }}>
                      ...
                      ：
                      {(size)}
                      {/* {`${downSize / 100}m底柜${topSize ? (` x ${topSize / 100}m顶柜`) : ''} `} */}
                    </div>
                    <div style={{ marginTop: '5px', marginLeft: '20px' }}>
                      <Trans>组合价格</Trans>
                      ：
